...
public class ValidateCouponQueryHandler : IRequestHandler<ValidateCouponQuery, Result<CouponValidationResultDTO>>
{
    ...
    public async Task<Result<CouponValidationResultDTO>> Handle(ValidateCouponQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (coupon == null)
            {
                return Result.Success(new CouponValidationResultDTO(false, "Invalid coupon code"));
            }

            if (!coupon.IsActive || coupon.StartDate > DateTime.UtcNow || coupon.EndDate < DateTime.UtcNow)
            {
                return Result.Success(new CouponValidationResultDTO(false, "Coupon is expired or inactive"));
            }

            if (coupon.UsageLimit.HasValue && coupon.UsageCount >= coupon.UsageLimit.Value)
            {
                return Result.Success(new CouponValidationResultDTO(false, "Coupon usage limit reached"));
            }

            if (coupon.MinimumOrderAmount.HasValue && request.OrderTotal < coupon.MinimumOrderAmount.Value)
            {
                return Result.Success(new CouponValidationResultDTO(false, $"Minimum order amount of {coupon.MinimumOrderAmount.Value} not met"));
            }
            ...
            return Result.Success(new CouponValidationResultDTO(true, "Coupon is valid", discountAmount));
        }
        catch (Exception ex)
        {
            return Result.Failure<CouponValidationResultDTO>(new Error("Discount.ValidationFailed", $"Failed to validate coupon: {ex.Message}"));
        }
    }
}
